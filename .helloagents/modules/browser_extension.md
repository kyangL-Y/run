...
- `popup.html` / `popup.js` 是右上角默认入口和页面浮层共用的本地完整工作台：
  - Popup 配置归一化必须保留 `authToken`、`authUser`、`currentShop`、`shops` 和 `authenticated`，否则页面内 iframe embedded 模式会误判为未登录并停在“打开设置页”闸门。
  ...
  - 轻量动作直接执行。
  - 基础功能里的“竞对建议价”主流程仍按不同房型结合竞对价生成建议价，用户可逐条修改最终价，并在提交前查看确认摘要；确认后才会推送 OTA。
  - 基础功能的默认商家改价目标页仍保留飞猪 `https://hotel.fliggy.com/ebooking/hotelBaseInfoUv.htm#/ebk-rp/roomsVsManage` 作为历史兼容兜底，但不再是商家改价主工作流入口。
...
- 商家改价板块已独立为“三平台价格读取 -> 映射 -> 确认提交”工作流：
  - Popup 提供飞猪、携程、美团三个商家价格页 URL 输入框；点击读取后，background 按平台打开或接管对应已登录后台页，采集当前页房型价并生成三平台映射预览。
  - 映射预览支持按平台勾选提交项、编辑最终价、按当前价一键填充，并在提交前给出分平台确认摘要；平台格子优先展示清洗后的房间介绍并采用紧凑三列布局，避免把抓取后的价格作为主要内容，也减少单个映射组占用空间。
  - 提交阶段通过 `MERCHANT_PLATFORM_MAPPING_SUBMIT` 按平台分组调用现有本地商家后台 DOM 回填器，保留二次确认，不自动提交未勾选项。
...
- 页面浮层的布局也与 Popup 收敛：使用“基础功能 / 商家改价”分组展示，基础功能内再使用“配置竞对房型价 / 竞对建议价 / 快捷动作”横向子切换；商家区也改为和 Popup 一致的“三平台映射读取 + 确认提交 + 高级备用统一目标价”层级；面板尺寸按 Popup 设置为 `400px × 780px`，标题区、状态卡、卡片、导航、按钮、输入框和工作台底部“返回结果”位置按 Popup 主排版对齐，避免页面内运营助手继续停留在旧版纵向堆叠样式。
- 当前页面浮层的业务界面通过 iframe 加载本地完整 `popup.html?embedded=1`；`content.js` 保留页面内启动按钮、浮层容器、当前页识别、当前页采集、本地商家房型读取、本地回填和协议转发等能力。
- 页面浮层外层 fixed 面板使用 `top: 20px` 与 `bottom: 96px` 自适应高度，避免在窗口高度不足时用固定 `780px` 高度把顶部标题挤出可视区；移动窄屏同理使用 `top: 10px` 与 `bottom: 86px`。
- 页面浮层每次展开时会将 iframe 滚动位置重置到顶部；embedded Popup 自身也禁用 `scrollRestoration` 并在加载后回顶，避免复用 `popup.html?embedded=1` 时沿用旧滚动位置导致标题和说明被截掉。
...
- `bridge.html` 降级桥只接受扩展自身与受支持业务平台 origin 的请求；`content.js` 向 bridge iframe 发送消息时指定扩展 origin，接收响应时也校验扩展 origin，避免 bridge 通道继续使用通配符 `postMessage`。
...
- 页面浮层的商家改价主流程与 Popup 同步提供“三平台价格页读取 -> 映射 -> 确认最终价 -> 提交”；其中“读取当前页房型价”使用当前标签页链路读取本店房型价，避免页面浮层继续依赖隐藏商家连接 URL。
...
- `background.js` 是统一代理层：负责配置读写、Bearer Token 注入、店铺切换、竞对酒店配置数据库读写、采集参数拼装、商家改价动作代理、竞对酒店详情页房型价抓取代理、独立竞对建议价请求拼装和运行时消息分发；插件 alarm 仅定时拉取云端趋势接口并基于云端趋势快照触发通知，不再本地打开飞猪页面执行采集。
  - 商家改价现在优先走本地已登录浏览器接管：`background.js` 根据 Popup 传入的三平台 URL 打开或接管对应商家后台页，分别读取房型价后生成三平台映射快照，并在确认后按平台调用本地 DOM 回填器完成真实提交；`MERCHANT_PRICING_*` 保留作兼容入口，主流程改为 `MERCHANT_PLATFORM_PRICE_SNAPSHOTS` / `MERCHANT_PLATFORM_MAPPING_SUBMIT`。
...
- 页面内商家改价入口已切换为三平台独立模式：读取、映射和提交均围绕飞猪、携程、美团的商家价格页 URL 展开，基础功能仍保持旧的竞对抓价和建议价入口。
...
- `browser-extension/bridge.js`：在内容脚本降级链路下继续携带 Bearer Token 与当前店铺上下文访问后端接口，并校验 bridge 请求来源与响应目标。
